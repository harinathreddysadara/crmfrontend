<template>
  <div class="grid-container">
    <div class="item1">
        <h1>Add Description and upload for Assigned Ticket</h1>
      <p>TicketId: {{getTicket.ticketId}}</p>
      <p>TIcket Description: {{getTicket.descriptionOfPost}}</p>
      <p>TIcket Status: {{getTicket.status}}</p>
   <!-- <p>Lead Assigned Time{{format_date(getTicket.getAssignedTime)}}</p> -->
      <p>Ticket Request Time: {{format_date(getTicket.requestTime)}}</p>
      <form>
          ...
          <div v-if="imageData!=null">
            ...
            <br><br>
              <router-link to="/historySupport">
              <v-btn style="width:160px;margin-left:50px;padding:5px;background-color: green;color:white"  @click="submit()">SUBMIT</v-btn>
            </router-link>
...
export default {
  ...
  methods: {
    ...
    previewImage(event) {
     ...
     submit(){
              this.$router.push('/historySupport')
    },
      ...
      ()=>{this.uploadValue=100;
        storageRef.snapshot.ref.getDownloadURL().then((url)=>{
          ...
          let data ={
              ...
              supportAgentId:localStorage.getItem('agentId'),
              ticketId:localStorage.getItem('leadId'),
              comments:this.comments

          }
          this.$store.dispatch('SendTicketDetails',data)
            // params:{
            //   uploadimageurl:this.image,
  ...
  created() {
    ...
    this.$store.dispatch("getSupportAgentTicketDetails", {
